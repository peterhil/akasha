...
class OscInitTest(unittest.TestCase):
    ...
    def testSize(self):
        """Test size of roots"""
        self.assertEqual(8, len(Osc(1,8).sample(times_at(xrange(8)))))
...
class OscRootsTest(unittest.TestCase):
    ...
    def testRootFuncSanity(self):
        ...
        wi = 2 * pi * 1j
        a = Osc(1,8).sample(times_at(xrange(8)))
        b = np.array([
            +1+0j, exp(wi*1/8),
            +0+1j, exp(wi*3/8),
            -1+0j, exp(wi*5/8),
    ...
    def testPhasors(self):
        ...
        for period in (5,7,8,23):   # + tuple( random.choice(range(1, 44100)) )
            ...
            a = to_phasors(o.frames(xrange(o.period)))
...
class OscSlicingTest(unittest.TestCase):
    ...
    def testListAccess(self):
        self.assertEqual(*self.o.frames(-1,5,11))
        self.assertEqual(*self.o.frames(0,6,12))

    def testSliceAccess(self):
        # assert np.equal(self.p[::], self.p.frames(xrange(8))).all()
        assert np.allclose(self.o[:3:2], Osc(1, 3).frames(xrange(3)))
