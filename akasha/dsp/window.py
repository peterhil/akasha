...
def sliding_window(a, ws, ss=None, flatten=True):
    ...
    if 1 != len(set(ls)):
        raise ValueError(
            'a.shape, ws and ss must have the same lengths. '
            f'They were {ls!s}'
        )
    ...
    slices = np.ceil((shape - (ws - ss)) / ss.astype(np.float))
    newshape = norm_shape(slices)
    ...
    # pad with step size (ss) zeros on each dimension in order to not get
    # garbage from the last indices
