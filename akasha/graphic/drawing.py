...
def clip_samples(signal):
    ...
    if clip_max > 1.0:
        logger.warning(
            "Clipping signal -- maximum magnitude was: %0.6f",
            clip_max
        )
        return clip(signal)
    return signal
...
def add_alpha(rgb, opacity=255):
    ...
    """
    alpha = np.array(
        [opacity] * len(rgb), dtype=np.uint8
    ).reshape(len(rgb), 1)
...
def draw_lines_pg(
    signal, screen, img, size=1000,
    colours=True, antialias=False
):
    """
    ...
    if colours:
        ...
        getattr(pygame.draw, method + 's')(
            screen,
            pygame.Color('orange'), False, pts, 1
        )
    return None
...
def draw_lines(signal, img, size=1000, colours=True, antialias=False):
    ...
    points = np.rint(
        get_points(flip_vertical(signal), size) - 0.5
    ).astype(np.uint32).T
